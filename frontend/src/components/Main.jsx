...
export const Main = () => {
    ...
    return (<>
        <div style={classes.headerOuter}><ConnectWallet style={classes.headerInner} /></div>
        <div style={{margin: '10px'}}>
            <CreateProposal style={classes.createProposal} proposals={proposals} setCodes={setCodes} />
            <h1>Proposals</h1>
            <ul style={{paddingInlineStart:'unset'}}>
            
            {proposals.map((proposal) => <Proposal proposal={proposal} classes={classes} />)}
            </ul>
        </div>
    </>);
}